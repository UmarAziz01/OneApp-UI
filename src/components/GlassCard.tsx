...
// Hapus deklarasi ganda, gunakan langsung yang ini dengan tipe GlassCardProps
const GlassCard = ({
  children,
  intensity = 20,
  tint,
  ...
  const containerStyle = useMemo(
    () => ({
      ...
      width: isDesktop ? Math.min(width * 0.5, 520) : '100%',
      alignSelf: 'center',
      ...Platform.select({
        web: {
          boxShadow: isDark
  ...
  return (
    <View style={containerStyle as any}>
      {/* Inner specular highlight - top edge light leak */}
      {isDark && (
        <View
          style={{
            position: 'absolute',
            ...
            backgroundImage: Platform.OS === 'web'
              ...
              : undefined,
            zIndex: 1,
          } as any} // Tambahkan 'as any' di sini untuk bypass TS error backgroundImage
        />
      )}
      ...
      <BlurView intensity={isDark ? intensity : intensity * 1.5} tint={blurTint} style={styles.blur}>
        <View style={[styles.content, !noPadding && { padding: padding || 24 }]}>
          {children}
        </View>
      </BlurView>
    </View>
